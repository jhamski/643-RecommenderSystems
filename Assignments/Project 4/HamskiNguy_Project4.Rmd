...
smp_size <- floor(0.75 * nrow(user.artists.matrix))

set.seed(123)
train_ind <- sample(seq_len(nrow(user.artists.matrix)), size = smp_size)

user.artists <- user.artists.matrix[train_ind, ]
user.artists.test <- user.artists.matrix[-train_ind, ]
#reduce number of users to make calcs faster
...
```{r}
user.IDs <- rownames(user.artists)
