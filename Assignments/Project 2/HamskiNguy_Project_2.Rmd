...
```{r, eval=FALSE}
tags.by.artist <- user.taggedartist %>% group_by(tagID, artistID) %>% tally()
#artist.tags.sparse <- sparseMatrix(tags.by.artist$tagID, tags.by.artist$artistID, x = tags.by.artist$n)

# http://stackoverflow.com/questions/9617348/reshape-three-column-data-frame-to-matrix
x <- tags.by.artist$tagID
y <- tags.by.artist$artistID
z = tags.by.artist$n

artist.tags <- with(tags.by.artist, sparseMatrix(i = as.numeric(x), j=as.numeric(y), x=z, dimnames=list(levels(x), levels(y))))

artist.tags <- as.matrix(artist.tags)
```

```{r, eval=FALSE}
length(tags.by.artist$n)
nnzero(tags.by.artist)
```


```{r, eval=FALSE}
dim(artist.tags)
length(unique(tags.by.artist$tagID))
```
...
This tutorial notes that we can 'boost' more obscure terms by using a square root or log transformation on word count frequencies instead of an untransformed, linear 

```{r, eval=FALSE}
# chunk modified from: http://anythingbutrbitrary.blogspot.com/2013/03/build-search-engine-in-20-minutes-or.html
n.bands <- length(unique(tags.by.artist$artistID))
...
tfidf.matrix <- t(apply(artist.tags, 1, FUN = get.weights.per.term.vec))

colnames(tfidf.matrix) <- colnames(unique(tags.by.artist$artistID))

artist.tags[0:40,1:15]
```
...
query.vector <- rep(0, length(tfidf.matrix[,1]))
query.vector[73] <- 1
query.vector[84] <- 1
query.vector[821] <- 1
